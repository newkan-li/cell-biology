...
const Q = {
 ...
 "ch02_s1_3": "说明水的极性（氢键）对其跨膜运输方式的影响：为什么水主要经水通道跨膜？",
 "ch03_s0_8": "写出相对离心力 RCF 的公式，并计算转子半径 r=10 cm、转速 10000 rpm 时的 RCF。",
 "ch06_s2_5": "推导微管临界浓度 Cc 的表达式，并说明游离 tubulin 高于/低于 Cc 时微管的行为。",
 "ch06_s7_0": "已知 kinesin 每水解 1 个 ATP 前进约 8 nm，若测得速度 v=1 μm/s，求其步频与每秒 ATP 水解数。",
 "ch07_s1_5": "说明微丝发生踏车(treadmilling)的条件，并写出正、负端净伸长速率的表达式。",
 "ch09_s1_2": "写出群体倍增时间公式，并计算：24 h 内细胞数由 1×10⁵ 增至 4×10⁵ 时的 Td。",
 "ch09_s1_3": "简述用脉冲标记法(PLM)测定细胞周期各时相时长的原理与步骤。",
 "ch10_s3_1": "说明信号级联放大的估算方法，并计算三级放大倍数分别为 1∶10∶100 时的总放大倍数。",
 "ch12_s3_3": "若端粒每次细胞分裂约缩短 100 bp，初始约 12 kb，试估算其可继续分裂的次数（到临界长度）。",
 "tb03_s0_0": "写出显微镜分辨率公式，并计算 λ=550 nm、NA=1.4 时的分辨率（并说明与光镜分辨极限的关系）。",
 "tb03_s2_0": "写出细胞计数与稀释换算公式，并计算：血细胞计数板 4 个大方格共 200 个细胞、每格 0.1 mm³ 时的浓度。",
 "tb06_s0_7": "估算 1 分子葡萄糖经有氧氧化的净 ATP 产额（糖酵解 + TCA 循环 + 氧化磷酸化）。",
 "tb06_s1_10": "写出质子动力势 Δp 的表达式，并说明 ATP 合酶合成 ATP 的 H⁺ 计量（约几个 H⁺/ATP）。"
};
const out = {};
ids.forEach(id => {
  ...
  ch.modules.forEach(m => (m.slides || []).forEach(s => {
    ...
    if (!s.derive) return;
    out[k] = { q: Q[k] || ("请完成本页推导： " + String(s.title).replace(/\s+/g, " ")), a: String(s.derive).replace(/\s+/g, " ").trim() };
  }));
});
